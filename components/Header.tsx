  ...
  return (
    <div className="relative flex items-center h-14 lg:h-[72px]">
      ...
      <nav className="z-10 fixed top-0 flex items-center bg-white/90 h-14 lg:h-[72px] lg:px-7 rounded-b-[20px] backdrop-blur-md shadow-customShadow hover:shadow-customShadowHover transition ease-out duration-150 dark:bg-charcoal">
        ...
        <button
          className="desktop-menu-toggle hidden lg:inline-flex group"
        ...
        <button
          onClick={handleNavToggle}
          ref={toggleRef}
          className={`mobile-nav-toggle inline-flex justify-center items-center group w-14 h-14 lg:hidden ${
            isNavVisible ? "active" : ""
          }`}
        >
        ...
        {/* Nav */}
        <ul
          className={`nav-list absolute top-16 left-0 bg-white min-w-52 p-6 rounded-[20px] space-y-2 invisible opacity-0 transition-all ease-linear duration-100 shadow-customShadow lg:shadow-none lg:visible lg:opacity-100 lg:static lg:top-auto lg:left-auto lg:space-x-5 lg:space-y-0 lg:bg-transparent lg:p-0 lg:rounded-none font-outfit font-medium uppercase text-sm tracking-[0.5px] dark:bg-charcoal dark:lg:bg-transparent ${
            isNavVisible ? "show" : ""
          }`}
          ref={menuRef}
        >
          {headerData.navigation.map((item: { name: string; href: string }) => (
        ...
        <div
          className={`desktop-menu hidden lg:inline-block absolute top-20 left-0 bg-white p-7 rounded-[20px] w-[300px] invisible opacity-0 shadow-customShadow hover:shadow-customShadowHover transition ease-out duration-150 dark:bg-charcoal ${
            isNavVisible ? "show" : ""
          }`}
          ref={menuRef}
        >
          ...
          <div className="mt-4">
            <h6
              id="email"
              className="font-outfit font-semibold dark:text-white"
            >
              Email:
            </h6>
            ...
            {headerData.socialLinks.map((item, index) => (
              <li key={index} className="list-none inline-block">
                <Link
                  ...
                  className="text-charcoal hover:text-black dark:text-white dark:hover:text-white"
                >
